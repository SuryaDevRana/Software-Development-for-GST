<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="font-bold text-3xl text-gray-900 dark:text-white">Add New Customer</h2>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Register a new customer in the system</p>
        </div>
    </x-slot>
            ...
            <form method="POST" action="{{ route('customers.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="address" placeholder="Enter full address" required rows="3"
                              class="w-full px-4 py-3 border-2 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition duration-300">{{ old('address') }}</textarea>
                    @error('address')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
